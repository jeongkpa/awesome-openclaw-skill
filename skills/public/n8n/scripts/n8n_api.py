...
class N8nClient:
    ...
    def update_workflow(self, workflow_id: str, workflow_data: Dict) -> Dict:
        """Update existing workflow using the official v1 API."""
    ...
    def activate_workflow(self, workflow_id: str, version_id: str = None) -> Dict:
        """Activate/publish a workflow using the official v1 API."""
        payload = {}
        if version_id:
            payload['versionId'] = version_id
        return self._request('POST', f'workflows/{workflow_id}/activate', json=payload)
    
    def deactivate_workflow(self, workflow_id: str) -> Dict:
        """Deactivate a workflow using the official v1 API."""
    ...
    def execute_workflow(self, workflow_id: str, data: Dict = None) -> Dict:
        ...
        The current public v1 OpenAPI spec does not expose a generic
        `/workflows/{id}/execute` endpoint. For n8n Cloud, prefer:
        - activating a webhook-triggered workflow and calling its webhook URL, or
        - retrying an existing execution when appropriate.
        """
        raise NotImplementedError(
            "Direct workflow execution is not available through the current n8n public v1 API. "
            "Use a webhook trigger + activate workflow, or another trigger-specific execution path."
        )
...
def main():
    ...
    parser.add_argument('action', choices=[
        'list-workflows', 'get-workflow', 'create', 'update', 'activate', 'deactivate',
