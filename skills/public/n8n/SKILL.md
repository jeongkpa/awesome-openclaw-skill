...
#### Update Workflows
```bash
# Replace an existing workflow definition using official PUT semantics
...
> On current n8n Cloud/public v1 API, activation uses the publish endpoints internally:
...
python3 scripts/n8n_execution.py show --id <execution-id> --pretty
```

#### Execution Helper Views
```bash
# Recent execution summaries
python3 scripts/n8n_execution.py recent --workflow-id <workflow-id> --limit 10 --pretty

# Only failed runs
python3 scripts/n8n_execution.py recent --workflow-id <workflow-id> --status error --limit 10 --pretty
```
...
# Test webhook
python3 scripts/n8n_webhook.py --path <webhook-path> --test --data-file payload.json --pretty
```

Webhook workflow rule:
- Set the Webhook trigger's `responseMode` to `responseNode`
- End the workflow with `Respond to Webhook`
- Build the final payload in a Set/Code node immediately before `Respond to Webhook`

#### Manual Execution
```bash
# Current public v1 API does not expose a generic direct execute endpoint.
# Prefer a webhook-triggered workflow, then call its webhook URL after activation.
```
...
# Execute workflow
# For Cloud/public v1, trigger webhook-based workflows with scripts/n8n_webhook.py
```
...
tester.py dry-run --id <workflow-id> --data-file test.json`
