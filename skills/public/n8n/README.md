...
# For webhook workflows, call the webhook URL after activation
# Then monitor executions
...
# Update from file
python3 scripts/n8n_api.py update --id <id> --from-file workflow.json

# File-based export/edit/apply flow
python3 scripts/n8n_workflow_file.py export --id <id> --out tmp/workflow.json --clean
python3 scripts/n8n_tester.py validate --file tmp/workflow.json --pretty
python3 scripts/n8n_workflow_file.py apply --id <id> --file tmp/workflow.json --pretty
...
python3 scripts/n8n_execution.py show --id <exec-id> --pretty

# Recent summary view
python3 scripts/n8n_execution.py recent --workflow-id <id> --limit 10 --pretty
python3 scripts/n8n_execution.py recent --workflow-id <id> --status error --limit 10 --pretty

# Trigger a webhook workflow after activation
python3 scripts/n8n_webhook.py --path <webhook-path> --data '{"key":"value"}' --pretty

# Then inspect executions
python3 scripts/n8n_api.py list-executions --id <id> --limit 10 --pretty
...
# For Cloud/public v1, use scripts/n8n_webhook.py for webhook-triggered runs
```
...
9. **For webhook workflows, always use `Respond to Webhook`:** Set the trigger to `responseNode`, build the final response object, and end with an explicit `Respond to Webhook` node so callers consistently see the final result
